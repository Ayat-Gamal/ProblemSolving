...
class Solution {
  /**
    1- sort dim of each cuboid 
    2 - all oreinttions {Box class to rebresent each cuboid l,h,w} one is enought as we sort it
    ...
    4 - traverse over the array with i, j ,, wih this conidtion
        condition => arr[i].l < arr[j].l && arr[i].w < arr[j].w && lis[i] < lis[j] + arr[i].h
 ...
 if (n == 0) return 0;
  ...
  Arrays.sort(cuboids, (a, b) -> {
        ...
        Box[] arr = new Box[n];
        for (int i = 0; i < n; i++) {
             arr[i] = new Box(cuboids[i][0], cuboids[i][1], cuboids[i][2]);
        }
